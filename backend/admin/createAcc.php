...
if(isset($_POST["gender"]) && isset($_POST["fname"]) && isset($_POST["mname"]) &&
   ...
   isset($_POST["role_id"])) {
    ...
    $branch_id = isset($_POST["branch"]) ? $_POST["branch"] : 1; // Check for branch
    ...
    if($result->num_rows > 0) {
        ...
        $query2 = "INSERT INTO tbl_account (ac_username, ac_email, ac_password, role_id, branch_id)
                   VALUES(?, ?, ?, ?, ?)";
        $stmt2 = $conn->prepare($query2);
        $stmt2->bind_param("sssii", $username, $email, $hashedPassword, $role_id, $branch_id);
        $stmt2->execute();
